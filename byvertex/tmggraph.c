...
double tmg_distance_latlng(tmg_latlng *p1, tmg_latlng *p2) {
  ...
  return acos(cos(rlat1)*cos(rlng1)*cos(rlat2)*cos(rlng2) +
		   ...
		   sin(rlat1)*sin(rlat2)) * EARTH_RADIUS;
}
...
/*
  struct for our breadth-first search for connected components
*/
typedef struct tmg_edge_to_list {
  tmg_edge *via;
  tmg_vertex *to;
  struct tm_edge_to_list *next;
} tmg_edge_to_list;

/*
  Compute the number and sizes of the connected components of
  the graph.

  Caller is responsible for calling free on the pointer returned
  in the sizes parameter.
*/
void tmg_conn(tmg_graph *g, int *count, int **sizes) {
  ...
  //tm_edge_to_list *stack;
  sll *stack = create_sll();
  sll *size_list = create_sll();
  
  while (num_visited < g->num_vertices) {

    (*count)++;
    int component_size = 0;
    
    // pick the first unvisited vertex as a starting point
    vnum = 0;
    while (visited[vnum]) vnum++;
    sll_add_to_head(stack, vnum);
    
    // process vertices from the stack as long as it's not empty
    while (!sll_isempty(stack)) {
      vnum = sll_remove_from_head(stack);
      if (!visited[vnum]) {
	// visit!
	visited[vnum] = 1;
	num_visited++;
	...
	// check on our neighbors, push any unvisited
	elist = g->vertices[vnum]->edges;
	while (elist) {
	  // find the vertex at the end that's not the one
	  // where we are
	  int other = elist->edge->end1->vertex_num;
	  if (other == vnum) {
	    other = elist->edge->end2->vertex_num;
	  }

	  if (!visited[other]) {
	    sll_add_to_head(stack, other);
	  }
	  elist = elist->next;
	}
      }
    }
    sll_add_to_head(size_list, component_size);
  }

  *sizes = sll_to_array(size_list);
  sll_destroy(size_list);
  sll_destroy(stack);
  free(visited);
}

/* closest pairs of vertices */
void tmg_closest_pair(tmg_graph *g, int *v1, int *v2, double *distance) {
